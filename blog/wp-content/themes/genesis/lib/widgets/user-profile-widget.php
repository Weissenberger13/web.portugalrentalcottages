...
class Genesis_User_Profile_Widget extends WP_Widget {
	...
	?>
		...
		<p><label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Gravatar Size'); ?>:</label>
			<select id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>">
				...
				foreach ( (array)$sizes as $label => $size ) { ?>
					<option style="padding-right:10px;" value="<?php echo $size; ?>" <?php selected($size, $instance['size']); ?>><?php printf('%s (%spx)', $label, $size); ?></option>
				<?php } ?>
			</select></p>
		...
		<p>
			...
			<textarea id="<?php echo $this->get_field_id('bio_text'); ?>" name="<?php echo $this->get_field_name('bio_text'); ?>" style="width: 95%;" rows="6" cols="4"><?php echo htmlspecialchars( $instance['bio_text'] ); ?></textarea>
		</p>
