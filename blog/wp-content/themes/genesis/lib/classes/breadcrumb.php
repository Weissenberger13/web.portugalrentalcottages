...
class Genesis_Breadcrumb {
	...
	function get_home_crumb() {
		...
		$url = ( 'page' == $this->on_front ) ? get_permalink( get_option( 'page_on_front' ) ) : home_url();
		...
		return apply_filters( 'genesis_home_crumb', $crumb );
	...
	function get_blog_crumb() {
		...
		return apply_filters( 'genesis_blog_crumb', $crumb );
	...
	function get_search_crumb() {
		...
		return apply_filters( 'genesis_search_crumb', $crumb );
	...
	function get_404_crumb() {
		...
		$crumb = $this->args['labels']['404'] . '"' . $wp_query->query_vars['name'] . '"';
		
		return apply_filters( 'genesis_404_crumb', $crumb );
	...
	function get_page_crumb() {
		...
		return apply_filters( 'genesis_page_crumb', $crumb );
	...
	function get_archive_crumb() {
		...
		if ( is_category() ) {
			$crumb = $this->get_term_parents( get_query_var( 'cat' ), 'category' );
		} elseif ( is_tag() ) {
			/** adds WP 3.0 compatibiliy **/
			if ( function_exists( 'single_term_title') ) {
				$crumb = $this->args['labels']['tag'] . single_term_title( '', false );
			} else {
				$term = $wp_query->get_queried_object();
				$crumb = $this->get_term_parents( $term->term_id, $term->taxonomy );
			}
		} elseif ( is_tax() ) {
			$term = $wp_query->get_queried_object();
			$crumb = $this->get_term_parents( $term->term_id, $term->taxonomy );
			...
			$crumb = esc_html( post_type_archive_title( '', false ) );
		}

		return apply_filters( 'genesis_archive_crumb', $crumb );
	...
	function get_single_crumb() {
		...
		return apply_filters( 'genesis_single_crumb', $crumb );
