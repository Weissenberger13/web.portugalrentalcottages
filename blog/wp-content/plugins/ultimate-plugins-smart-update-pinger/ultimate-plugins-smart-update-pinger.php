...
function SUP_show_options_page(){
   ...
   echo '<div class="wrap">
	...
	<p>Click <a target="_blank" href="http://ultimateplugins.com/smart-update-pinger/installation-instructions/" title="Ultimate Plugins Smart Update Pinger Installation Instructions">here</a> for installation instructions</p>
	<p>Click <a target="_blank" href="http://ultimateplugins.com/smart-update-pinger/usage-instructions/" title="Ultimate Plugins Smart Update Pinger Installation Instructions">here</a> for usage instructions</p>
	<p>Click <a target="_blank" href="http://ultimateplugins.com/smart-update-pinger/" title="Ultimate Plugins Smart Update Pinger Updates">here</a> for updated versions</p>
	<p>Click <a target="_blank" href="http://ultimateplugins.com/smart-update-pinger/comments-suggestions/" title="Ultimate Plugins Smart Update Pinger Comments & Suggestions">here</a> for comments and suggestions</p>
...
function SUP_ping_if_new($id){
   ...
   if(get_option('SUP_ping') == 1 && trim(get_option('ping_sites')) != ""){
    ...
    if (trim(get_option('ping_sites')) != ""){
	   ...
	   $SUP_output_log=strftime("%D %T")."\t<strong>NOT Pinging services (<font color=\"#FF0000\">WARNING: EMPTY PING SERVICES LIST</font>)</strong>\n".$SUP_output_log;
	}  
   }
...
function SUP_get_last_log_entries($num){
   ...
   if(get_option("SUP_error") == 1){
    ...
    if($lines === false){
	 echo "Error reading log file (".$logfile."). <strong>Most likely you have manually removed the log file</strong>, or alternatively this could mean that the logfile (".$logfile.") is read-protected (change the rights of this file to 777), or that you have changed the directory or file name of the plugin (they both should be 'ultimate-plugins-smart-update-pinger')";
    }else{
      // Original: $lines = array_slice($lines, count($lines) - $num);
      // Modified to show in reverse order (easier for reading)
      $lines = array_reverse(array_slice($lines, count($lines) - $num));
